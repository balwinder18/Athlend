...
export const getDBSnapshot = async () => {
  ...
  const bookings = await Bookings.find({ bookingdate: { $gte: todayISO } }).lean();
...
7. When listing items, always use a **numbered list format** where each item appears on a new line. 
   Example:
   1. Ground A  
   2. Ground B  
   3. Ground C
8. Do NOT put multiple list items on the same line.
9. Do NOT mention 'database' or 'data' in your answer.
