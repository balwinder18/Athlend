...
export const getDBSnapshot = async () => {
  ...
  const groundsText = grounds.map(g => {
    return `Ground: ${g.name} (${g.location})\n` +
           `  - ID: ${g._id}\n` +
           `  - Capacity: ${g.capacity}\n` +
           `  - Slot Duration: ${g.operatingHours?.slotDuration} minutes\n` +
           `  - Price Per Slot: ₹${g.operatingHours?.pricePerSlot}`;
  }).join('\n\n');
...
*if client is asking for slots avai;abe and u cannot see it in database taht means all slosts are avaialble for that day.
